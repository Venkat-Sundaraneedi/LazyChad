...
function M.formatexpr()
  if LazyChad.has("conform.nvim") then
...
function M.info(buf)
  ...
  LazyChad[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    LazyChad.warn("No formatter available", { title = "LazyChad" })
  end
end
...
function M.health()
  ...
  local has_extra = vim.tbl_contains(Config.spec.modules, "lazychad.plugins.extras.lsp.none-ls")
  if has_plugin and not has_extra then
    LazyChad.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in LazyChad.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `lazychad.plugins.extras.lsp.none-ls` extra,",
...
function M.snacks_toggle(buf)
  return Snacks.toggle({
    ...
    get = function()
      ...
      return LazyChad.format.enabled()
    end,
    set = function(state)
      LazyChad.format.enable(state, buf)
    end,
  })
end
