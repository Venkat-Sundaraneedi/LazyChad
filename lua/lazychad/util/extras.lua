...
---@class lazychad.util.extras
local M = {}
M.buf = 0

---@type LazyExtraSource[]
M.sources = {
  { name = "LazyChad", desc = "LazyChad extras", module = "lazychad.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("lazychad.extras")
---@type string[]
M.state = nil
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #LazyChad.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function M.get()
  ...
  for _, source in ipairs(M.sources) do
    local root = LazyChad.find_root(source.module)
    if root then
      LazyChad.walk(root, function(path, name, type)
...
function M.get_extra(source, modname)
  LazyChad.plugin.handle_defaults = false
  ...
  return {
    ...
    managed = vim.tbl_contains(LazyChad.config.json.data.extras, modname) or not enabled,
    plugins = plugins,
    optional = optional,
  }
end
...
function X.new()
  ...
  self.float = Float.new({ title = "LazyChad Extras" })
...
function X:toggle()
  ...
  for _, extra in ipairs(self.extras) do
    if extra.row == pos[1] then
      ...
      LazyChad.config.json.data.extras = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, LazyChad.config.json.data.extras)
      M.state = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, M.state)
      if extra.enabled then
        table.insert(LazyChad.config.json.data.extras, extra.module)
        M.state[#M.state + 1] = extra.module
      end
      table.sort(LazyChad.config.json.data.extras)
      LazyChad.json.save()
      LazyChad.info(
        "`"
          .. extra.name
          ...
          .. "\nPlease restart LazyChad to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("LazyChad Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled LazyChad extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  local defaults = LazyChad.config.get_defaults()
  ...
  if not extra.managed then
    ...
    elseif vim.tbl_contains(LazyChad.plugin.core_imports, extra.module) or origin == "default" then
      self:diagnostic({
  ...
  if extra.source.name ~= "LazyChad" then
  ...
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^lazychad.plugins.extras.", "")
