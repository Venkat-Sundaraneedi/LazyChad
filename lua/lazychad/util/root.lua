...
function M.detectors.lsp(buf)
  ...
  local clients = LazyChad.lsp.get_clients({ bufnr = buf })
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  LazyChad.info(lines, { title = "LazyChad Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("LazyRoot", function()
    LazyChad.root.info()
  end, { desc = "LazyChad roots for the current buffer" })
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("lazychad_root_cache", { clear = true }),
...
function M.get(opts)
  ...
  return LazyChad.is_win() and ret:gsub("/", "\\") or ret
end
