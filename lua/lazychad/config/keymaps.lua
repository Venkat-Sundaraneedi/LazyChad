...
-- DO NOT USE `LazyChad.safe_keymap_set` IN YOUR OWN CONFIG!!
-- use `vim.keymap.set` instead
local map = LazyChad.safe_keymap_set
...
map({ "n", "v" }, "<S-h>", "^", { desc = "move beginning of line" })
map({ "n", "v" }, "<S-l>", "$", { desc = "move end of line" })
...
map({ "i", "n", "s" }, "<esc>", function()
  vim.cmd("noh")
  LazyChad.cmp.actions.snippet_stop()
  return "<esc>"
...
-- toggle options
LazyChad.format.snacks_toggle():map("<leader>uf")
LazyChad.format.snacks_toggle(true):map("<leader>uF")
...
if vim.fn.executable("lazygit") == 1 then
  map("n", "<leader>gg", function() Snacks.lazygit( { cwd = LazyChad.root.git() }) end, { desc = "Lazygit (Root Dir)" })
  map("n", "<leader>gG", function() Snacks.lazygit() end, { desc = "Lazygit (cwd)" })
  map("n", "<leader>gf", function() Snacks.picker.git_log_file() end, { desc = "Git Current File History" })
  map("n", "<leader>gl", function() Snacks.picker.git_log({ cwd = LazyChad.root.git() }) end, { desc = "Git Log" })
...
map("n", "<leader>ft", function() Snacks.terminal(nil, { cwd = LazyChad.root() }) end, { desc = "Terminal (Root Dir)" })
map("n", "<c-/>",      function() Snacks.terminal(nil, { cwd = LazyChad.root() }) end, { desc = "Terminal (Root Dir)" })
map("n", "<c-_>",      function() Snacks.terminal(nil, { cwd = LazyChad.root() }) end, { desc = "which_key_ignore" })
