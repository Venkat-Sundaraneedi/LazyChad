return {
  -- edgy
  {
    ...
    opts = function()
      ...
      if LazyChad.has("neo-tree.nvim") then
        local pos = {
          filesystem = "left",
          buffers = "top",
          git_status = "right",
        ...
        local sources = LazyChad.opts("neo-tree.nvim").sources or {}
        for i, v in ipairs(sources) do
          table.insert(opts.left, i, {
            ...
            pinned = true,
            open = function()
              vim.cmd(("Neotree show position=%s %s dir=%s"):format(pos[v] or "bottom", v, LazyChad.root()))
            end,
          })
        end
      end
